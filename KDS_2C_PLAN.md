...
3. **State tree — a SEPARATE top-level node, never under `/orders`.** `/pickup_ready_notifications/{orderId}` with an honest lifecycle:
   ...
   - `read_error_at` + `read_error_reason` — written when the order READ itself threw (transient RTDB error) before eligibility could be classified. Diagnostic only; never claims, never sends. Written via the SAME guarded `transaction()` (aborts if `claimed_at`/`sent_at` exists). It exists because RTDB at-least-once does NOT guarantee a business re-attempt after the handler resolves cleanly — so a rare read failure leaves a durable, ops-visible trace instead of a silent missed ping (never rethrow: no retry config, and the stamp IS the recovery signal).
...
5. **Honest recovery states — Admin-only view (Codex R1#3, R2#1/#3).** The recovery view is **Admin-SDK / Cloud Logging** (and, if built later, a dispatcher-gated *callable*) — NOT a direct client RTDB read, because the §7 rules deny ALL client reads, dispatcher included. Interpretation:
   ...
   - `read_error_at` only (no `claimed_at`) ⇒ the handler couldn't READ the order (transient RTDB error) ⇒ NOT sent ⇒ safe to manually send after verifying. Guarded, so it never lands on an already-claimed/sent node.
