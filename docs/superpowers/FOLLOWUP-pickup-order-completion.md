...
**Why a KDS write is unavoidable (verified 2026-07-31):** the KDS "Completar" is stored **only in that device's `localStorage`** (`completedSet` ← `localStorage.xpizza_kds_completed`, `index.html:1586`/`:1645`) — it writes **nothing** to the DB ("ONLY empezar→preparing and listo→ready write /orders.status", `:2034`). `estado==='Archivado'` is just a client-side mapping of `order.status==='delivered'` (`:2721`), not a stored field. So completing a pickup today leaves **zero trace in shared data** — there is nothing for dispatch (or anyone) to read. The ONLY DB-level "done" signal is `status==='delivered'`, which pickups never reach. So the completion must be **published** on Completar; the minimal publish IS the one-line write below. (There is no "dispatch just reads it" without the KDS writing it.)

**1. The change — make "Completar" publish pickup completion (one line in the KDS).**
...
- **Dispatch then reads `status` — ZERO dispatch change** (`getPickupQueue` already excludes `delivered`); delivered-stats + Cerrados also start counting pickups correctly. This is exactly the "dispatch reads the status" model — it just requires the KDS to actually set that status, which it currently doesn't for pickups.
