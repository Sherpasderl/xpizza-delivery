...
3. **Every read-only subscription explicitly declared.** New reads (scheduled-orders; optional `order_timelines`; optional `order_predictions`) must be added as named, read-only subscriptions — no hand-waved reads. In this plan, **two declared read-only subscriptions are added, both no-writes**: **`subscribeToDriverCash`** (`driver_cash`, Task 10a cash bar) and **`subscribeToScheduledOrders`** (`orders` filtered to held `scheduled`/`releasing`, Task 10b Programados — held orders are excluded from `allOrders` by `filterLiveOrders`/`NON_LIVE_ORDER_STATUSES`). Task 9 uses only already-subscribed `orders`/`tasks`/`etaCache`. No other new subscription.
...
- [ ] **Step 2:** Move `#reconciliation-group` (→ Caja), `#delivered-group`+`#closed-search` (→ Cerrados), and the `#msg-list-*` containers (→ Comms) into their tab regions **by DOM relocation, keeping their ids** so `renderReconciliationSection`/`renderDeliveredSection`/`renderMessagesLists` still target them. Verify each still renders + its actions fire.
  - **Comms live-update (must-fix):** the incoming-messages subscription currently re-renders **only when the `#msg-modal` is open** (`index.html:~2537`). Once the lists live in a tab, change that gate to **"Comms tab is active"** (or re-render unconditionally) so new inbound/handled updates refresh the in-tab list, not just the badge.
  - **Harden the reused renderers while relocating (escaping fix):** `renderDeliveredSection` emits `data-order-id="${o.order_id}"` and `L${o.total}` **unescaped** (`~3451`), and `renderReconciliationSection` emits `L${o.total}` unescaped (`~3502`). `o.order_id` is client-supplied at intake → the raw `data-order-id` attribute is a latent attribute-injection vector. **Wrap `escapeHtml(o.order_id)` and `escapeHtml(String(o.total))`** in both (same class as Task 9's `displayOrderLabel` sweep). This makes the "all rendered fields through `escapeHtml`" claim true.
...
## Session revision R8 (2026-07-30) — Part-B on-device polish & function batch (Tasks 13-refine + 14–19)

Live iteration session (Xavier reacting on-device at `:8777`, worktree `xpizza-dispatch-redesign`). All work is a **client-render layer on top of Task 12's WIP commit `80ad8da`**, uncommitted, pending ONE codex-on-diff pass. Zero-write, `escapeHtml` on all dynamic content, every existing handler preserved — EXCEPT the money-adjacent Task-18 cancel entry point (flagged). Each ask below is compartmentalized as a task.

- **Task 13 — topbar (BUILT + codex-APPROVED `69cf25d`/`1845bf6`, then refined to full mockup parity):** v6 brand + restaurant pill, **KPI cluster now CENTERED + borderless with `.kdiv` hairline dividers** (was a bordered right-pushed tray), Auto-asignar as the mockup `.apill` + **sliding switch** (`.aa-sw`, was a dot), **borderless `.iconbtn`** buttons, **slate avatar**, mockup header gradient. + three approved additions beyond the static gate: live **"Sin asignar" amber cue** (`#kpi-pending .alert` off pending count), driver status **hover-reveal** (later superseded by Task 14's always-visible chips), custom map controls (→ Task 16). On-time% still deferred to §1b (slot shows **Entregados**, honest); no ⌘K.
- **Task 14 — driver card mockup-parity rebuild (`renderDriverNode`):** flat row → mockup `.drv` card — rounded **avatar + initial + inset status dot** (`.av .s`, replaces standalone dot), **two-line identity** (`.dn` name / `.dm` meta), **GPS chip** (`.gps` "vivo"/"4m", red-pulse when gps-dark), pastel `.da` call/msg/more (unchanged handlers), **`.dark-row`** "Sin señal — considerar reasignar" strip, inline **`.dt` task summary** line(s) collapsed → click-to-expand keeps the full `.driver-tasks`/`renderTaskRow` detail. **Card gradient + `--shadow` elevation + hover-lift** (mockup line 69, the "still looks different" fix). **No vehicle field exists → meta shows availability/order-count ("Libre"/"1 activa"/"Regresando"), NOT a fabricated "Moto"** (Xavier chose honest-over-mockup).
- **Task 15 — right-rail layout controls (Xavier-driven):** (a) roster **split into two independently-collapsible groups** — En turno / Fuera de turno (`drivers-tree-live`/`-off`, reusing the `.tree-group.collapsed` toggle pattern; wiring now queries `#right-rail` to cover both trees). (b) **tab strip moved to the TOP** of the rail, above the roster. (c) **tab tap-toggle** — tapping the active tab collapses its pane (hide the list); FIX: there was **no `.rtab-pane.hidden` CSS rule at all**, so `.hidden` never hid — added it. (d) **drag-to-resize both rails** — `--lw-size`/`--rw-size` on `:root` (grid column + `.rail-inner` width derive from it), clamped 260–620, **persisted to localStorage**, `.main.resizing` kills the transition while dragging, map re-fits via ResizeObserver, handle hidden when the rail is collapsed. Handle color is a **neutral hairline** (Xavier: no red/green).
- **Task 16 — custom minimalist v6 map controls:** replaced Google's default zoom/fullscreen chrome (`disableDefaultUI` + `zoomControl:false` + `fullscreenControl:false`) with a glass **zoom +/- pill** (RIGHT_BOTTOM) + **fullscreen toggle** (RIGHT_TOP, icon+aria swap), mounted via `map.controls[...].push`; existing top-left `#map-fit-btn` (fit restaurant+drivers+orders) restyled to match glass (handler untouched).
- **Task 17 — order-code hygiene + emoji sweep:** (a) the raw `PZSX…` order code is **out of every list** — `displayOrderLabel` returns friendly `#N`, else a compact `#<4-char>` fallback; the **full code moved to the detail modal** as a `Código` row (`orderCode()`). (b) cheap emoticons → line icons: **⏰ → `#i-clock`** (sched badge + queue), **⋯ → `#i-more`** (unassigned + task more-btns, centered), **★ removed** from Cobertura chips (gold `.cv.new` already marks them). Left `▴` (disclosure caret) + `✓` (WhatsApp chip) as legitimate monochrome typography.
- **Task 18 — detail-modal actions (⚠ MONEY-ADJACENT — needs codex money-gate):** modal was read-only + Call/Ver-en-mapa only. Added **Message** (`wa.me` deep-link) and **Cancelar pedido** (`.od-action-btn.danger`, only when status not cancelled/delivered/completed) → **reuses the vetted money-safe `confirmCancelOrder`/`XPD.cancelOrderRemote`** (voids charge / releases driver) — NO money-logic change, just a NEW entry point. Cancel is now reachable from any order incl. scheduled → flag for the money-gate ([[codex-gate-money-adjacent]]).
- **Task 19 — unified "En Fila" pending queue (surfaces pickup):** pickup orders (`order_type:'pickup'`) have **no delivery task** → were invisible in every delivery-keyed view. The "Pedidos" tab became **"En Fila"** — a unified queue with category groups **Entrega** (`getDeliveryQueue`, all non-terminal delivery) / **Recoger** (`getPickupQueue`, non-terminal pickup) / **Programados** (from `allScheduled`, now stored globally). The separate Programados tab was **folded in**. Every row taps → `openOrderDetailModal` (now also resolves `allScheduled`, since scheduled orders aren't in `allOrders`). Read-only render; the only actions are Task-18's modal buttons.

**R8 status / handoff:** Tasks 13–19 BUILT in the worktree, div-balanced, `node --check` clean, `:8777` 200, on-device-iterated with Xavier. **NOT yet committed, NOT yet codex-gated.** Next = ONE codex-on-diff pass over the full working-tree diff (`80ad8da`..worktree) — must include the **Task-18 money-gate** — then commit + squash the WIP, then **version bump `v1.5.8·dispatch`** = Phase 1 done. Task 12 (Comms thread, `80ad8da`) still needs its on-device sign-off in the same pass.
