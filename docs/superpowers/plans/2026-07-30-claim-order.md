...
## MONEY PLAN-GATE — APPROVE-WITH-CHANGES (folded in)
- **CHANGE 1 (token OPTIONAL; phone-match is the REQUIRED gate).** Scheduled orders have no `tracking_token` at success-time → a token-required claim can't retro-credit a scheduled order from the success card (a big after-hours segment). Phone-match alone is secure (uid is OTP-minted; a guessed order_id is useless without controlling the order's phone). So: **success-card claim = TOKENLESS** (order_id + phone-match; works ASAP + scheduled) → drops the `submitOrder` `tracking_token` capture (OQ3 gone). **Tracker deep-link claim = token path** (order_id + token↔order strict bind + phone-match, belt-and-suspenders).
- **CHANGE 2 (tokenless guards).** On the tokenless path, **uniform 403** for EVERY unclaimable case (missing order · wrong phone · already-claimed-by-another · cancelled/refunding · malformed) — no 404, no distinguishable reason, no guest/status oracle. Self-already-claimed → idempotent `ok`. The token-present path MAY keep stricter typed reasons. **Tokenless rate bucket keyed by verified uid** (new `claim_uid` bucket; + soft per-uid+phone) — the per-token `claim_token` bucket doesn't protect the tokenless enumeration path (resolves OQ2: per-uid bucket is REQUIRED).
- **CHANGE 3 (`bindDecision` cancel-in-progress fields — order-node ONLY).** Abort the bind if: `status === 'cancelled'` · `resolving_action === 'cancel' && cancel_claim_id` (the essential race field — `claimDecision` sets it in the order tx, cancel-order-core.js:101) · `payment_status ∈ { refunded, refund_pending, manual_review }`. Do NOT check `cancelling`/`cancel_pending` — those live under `payment_attempts`, not the order node (payment-callback guards, not the bind guard).
