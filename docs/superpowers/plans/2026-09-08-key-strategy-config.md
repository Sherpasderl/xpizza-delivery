...
- [ ] **Step 1: Failing test** — (a) publishing writes `key_mode` onto the version record AND the snapshot, taken from **`source.key_mode`** (the value the keys were built + validated against — NOT a fresh profile read, which could drift; codex blocker #1); a source without `key_mode` (legacy) writes nothing → serving fallback; (a2) **the publish-side `writeMirror` payload carries `keyMode`** (the Task-2 fix taught the mirror WRITER to persist it; publish must now hand it the mode so a freshly-published config version serves id-correct through the RTDB mirror on the very first outage — absent ⇒ omitted, byte-identical for both brands); (b) **rollback to a prior version re-emits its snapshot/mirror carrying that version's `key_mode`** (codex blocker #2 — rollback is production serving state; stripping keyMode would misprice a config merchant).
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement:**
  ...
  - **The publish-side `writeMirror` call (`:230`)** — currently mode-less (Task 2 deliberately left it so; the writer already omits absent) — now passes `keyMode: <source.key_mode>` so a first-publish config version's mirror carries the mode, matching the rollback path. Legacy source (no `key_mode`) ⇒ omitted ⇒ byte-identical.
  ...
  - **⚠️ BACKFILL (`tools/backfill-snapshot.js:38,39`) — the FOURTH write path (codex T3 BLOCK).** This owner-run tool reads the active version via `readVersionDocs` (→ returns `keyMode`) and **rewrites `active_snapshot` + the RTDB mirror** for it — and its header declares re-running MANDATORY (to add the `seq` ordinal). It currently drops `keyMode`: `snapshotOf(...)` omits the 6th arg and the `writeMirror` payload omits `keyMode`. For a config merchant this strips the mode from production fallback state while the version record keeps it → misprice on the ladder. **Fix:** pass `docs.keyMode` into `snapshotOf(rid, versionId, docs.seq, menuTable, extraTable, docs.keyMode)` AND the mirror payload via the same `...(docs.keyMode !== undefined ? { keyMode: docs.keyMode } : {})` spread. Byte-identical today (`docs.keyMode` undefined for both live brands). Guard it structurally in `publish-parity.test.js` alongside the `--from-store` guard (reads the VERSION not the profile; omit-when-absent). **Completeness rule: grep EVERY caller of `snapshotOf`/`writeMirror` — the write primitives — not just the publish/rollback call graph.**
