...
- **Rules (guard tests):** `/user_profiles/{uid}` readable/writable only by `auth.uid===uid`, denied for other/anon; wholesale client delete (null) denied; `$other:false`, `phone`/`created_at` immutable, `addresses` denied. `/otp`,`/otp_ip`,`/phone_index` deny all client access. **`/user_orders/{uid}` denied to anon, staff, AND the owning customer in P0** (owner-read opens only in P3 with the recycled-number control). **H1: a `customer:true` token is DENIED on `/orders`,`/tasks`,`/order_timelines`,`/config`,`/drivers`; existing staff/driver access unchanged.**
...
**H9 — Recycled-number: accepted phone-account model + bounded exposure (PRODUCT RISK — owner decision).** OTP proves *current control of the number*, not original ownership — so, exactly like every phone-first account (WhatsApp, Uber, DoorDash), a recycled number can inherit the account. A "¿Sos vos?" prompt is **not** a control (the new holder just taps "sí"). P0 takes the **accepted, disclosed** model, bounded so exposure is minimal: (a) **no PII shown before a successful OTP** (phone control proven first); (b) the only durable P0 field is a first name; (c) `/user_orders` is **read-denied in P0** — zero history exposure; (d) **inactivity aging** — a scheduled sweep wipes profiles dormant > ~6 months so a recycled number resolves to a fresh account (a risk *reducer*, not a proof). **Before P2/P3** expose addresses/order-history (higher-value PII), a **stronger anti-recycling control is a BLOCKING pre-req** — a customer-set recovery signal (email/PIN) gating durable data, or default-rotate-to-fresh-uid + support-assisted merge. **Whether to accept the phone-account model for P0 (recommended, industry-standard) vs add a second factor now is a documented risk-acceptance decision for the owner.**
