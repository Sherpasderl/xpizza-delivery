...
1. **Client sends a REQUEST only** — `redeem:{type:'free_pizza'}` (x_pizza) / `{type:'free_item', item_id}` (la_musa). Never a trusted amount/balance/discount.
   - **A redeem request REQUIRES a VERIFIED `customer_uid`.** It must NOT inherit the payment path's fail-open-to-guest attribution (the 1.5s token-verify deadline that treats timeout/error as guest so payment isn't blocked). On a redeem request, token timeout/failure → return a **non-payable** `409/401` "iniciá sesión / reintentá", NEVER a guest/full-price payable checkout. (R2 finding 4)
2. **Reserve (debit-first, idempotent by order_id + canonical fingerprint):** at intake (`createOrder` / `chargeOnlineOrder`), BEFORE computing the charge: transaction on `user_rewards/{uid}/{rid}` — verify `available ≥ cost` AND the reward applies to THIS server-priced cart (x_pizza: ≥1 pizza present → discount = cheapest pizza's cents; la_musa: tier item present/eligible), then move `cost` into `reserved` + write a `reserve` ledger entry keyed by `order_id`. **The reservation record stores `{order_id, restaurant_id, order_fingerprint, canonical_redemption_result, config_version, active_attempt_id?}`; same-`order_id` re-entry REUSES it ONLY if ALL those canonical fields match — a mismatch (different cart/reward before `orders/{id}` was written) is REJECTED, not silently reused.** (R2 finding 1)
   - **All-or-nothing, non-payable on any failure (R2 finding 2):** if `redeem` was requested and cannot be applied EXACTLY (insufficient `available`, ineligible cart, config_version changed, fingerprint mismatch, unverified uid) → return a **non-payable `409/400`** telling the client to resubmit (with or without the reward). NEVER return a payable full-price PixelPay URL after a reward was requested — a silent full-price charge is a money-path consent defect. (Full-price is only ever charged when NO redeem was requested.)
...
5. **Consume on completion / release on failure — tied to the attempt/order lifecycle (finding 2):**
   ...
   - **Cash reservations have no attempt TTL (R2 finding 3):** online binds release to `hosted_expires_at`; a CASH order (`createOrder`, no PixelPay attempt) needs its own stale-release policy — release a reservation whose cash order never reached `completed`/`cancelled` within an age window, and if the order is still live, surface to dispatcher audit before releasing (never strand `reserved` on an abandoned cash order, never release under an order still being fulfilled).
...
**Money invariants (gate hardest):** discount ALWAYS server-computed from the server-priced cart; client sets no discount/balance/eligibility; **a redeem request requires a VERIFIED customer_uid (no fail-open-to-guest)**; reserve is debit-first + idempotent-by-order_id AND bound to the canonical `{order_fingerprint, redemption_result, config_version}` (same-order_id reuse only on exact match); **redemption is all-or-nothing — any failure returns a NON-payable 409/400, never a payable full-price checkout after a reward was requested**; consume/release tied to the order/attempt lifecycle (online → hosted TTL; cash → age/status policy); never negative total; free-pizza requires a pizza in cart; fingerprint carries the canonical redemption; factura + `total_cents/subtotal_cents/tax` reconcile with the discount line; refund returns points once. `isCashPayment` + existing money/fingerprint logic byte-intact except the documented discount line.
