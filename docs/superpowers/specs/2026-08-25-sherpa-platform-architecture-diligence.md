...
- **Tier 1 — Flagship / owned (X. Pizza, La Musa):** the full deep stack — KDS, rewards/loyalty, deep order lifecycle. **Factura differs per Restaurant (corrected 2026-08-25, source-verified):** Sherpa-issued (real CAI) for **X. Pizza only** (`FACTURA_RESTAURANT_ID = 'x_pizza'`); **La Musa is `external_pos` — it issues its own factura via its Soft Restaurant POS**, so Sherpa allocates no ISV and stores no fiscal split for it. Unchanged.
