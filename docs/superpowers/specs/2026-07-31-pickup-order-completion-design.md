# Pickup-order completion — design spec (v6: `completed`, EXHAUSTIVE consumer matrix, on current main)

**Date:** 2026-07-31 · **Status:** REVISED (v6) — folds v5-regate's last find (ready-time-quality Set-membership, ⑧) + test-harness + the SDK-divergence correction. The dual-grep enumeration (equality + Set/list) is now the exhaustive set — v5 was the pass that closed it. **Base = `origin/main` `c09fe12`** (post-rewards-v2). Worktree **`feat/pickup-completion`**. Supersedes v5 (`574d39d`).
