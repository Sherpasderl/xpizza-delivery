# Pickup-order completion — design spec (v7: `completed`, EXHAUSTIVE matrix — DESIGN APPROVED)

**Date:** 2026-07-31 · **Status:** ✅ **DESIGN APPROVED** (advisor codex, 6 rounds). v6-regate's 3 "unbucketed" hits were all **already-correct LEAVE** behavior (no code change) — now listed, so §3 is provably complete. **Base = `origin/main` `c09fe12`** (post-rewards-v2). Worktree **`feat/pickup-completion`**. Next: writing-plans → build → codex-on-diff money-gate (the real remaining gate — first code) → deploy, folded into the rewards launch.
...
- **SDK live-filter `NON_LIVE_ORDER_STATUSES` / `filterLiveOrders`** (`order-filter.js` + `xpizza-delivery.js`, 5 copies — **NOTE: these copies DIVERGE**, they are NOT byte-identical: `xpizza-reference` = `{pending_payment}`; the 4 app copies (kitchen/dispatch/dashboard/driver) = `{pending_payment, scheduled, releasing}`). **DECISION: LEAVE — `completed` stays LIVE** (it's in *none* of the copies → live everywhere, exactly like `delivered`). Rationale: live orders reach `allOrders`; the matrix's dispatch closed-list (DI1) + dashboard completed-stats (D3–D6) **require `completed` orders to be present** to display them as terminal. Making `completed` non-live would filter them out of `allOrders` → contradict the design. **No SDK edit.** ⚠ **This LEAVE is safe ONLY because the §3A display fixes treat a live `completed` order as terminal-present** (statusBucket/pill/tracker/estado) — LEAVE depends on FIX; internally consistent once built.
- **`ready-time-quality.js:19` `TERMINAL = {out_for_delivery, delivered}`** (used by `classifyTerminal` :72): **DECISION: LEAVE.** It's a **PURE-SHADOW** predictor-quality monitor (no user/dispatch/order effect; the predictor writes ONLY `order_predictions`/`prediction_logs`/`ready_time_*`). Ready-time is predicted/measured at the **`ready`** transition (which pickups already hit); TERMINAL is a delivery-lifecycle dimension. A completed pickup being absent is **not a regression** (pickups never reached `out_for_delivery`/`delivered` before this change either) and doesn't affect `ready_at` accuracy. Adding `completed` would expand the functions footprint to a **2nd file** for a shadow-metric nicety → deferred as an optional follow-up.
- **`xpizza-reference/test-harness.html:522`** (treats `completed` as still-assignable): **OUT OF SCOPE** — non-prod test harness, not a live consumer.
- **`AUTO_ASSIGNABLE_STATUSES = {new,preparing,ready}`** (`index.js:3854`, used :4253): **LEAVE — already correct.** `completed` isn't auto-assignable (a fulfilled pickup must not be assigned a driver). Excluding it is the right behavior; no change.
- **`scheduled-orders.js NON_LIVE_STATUSES = {pending_payment,scheduled,releasing}`** (:25): **LEAVE — already correct.** `completed` stays live / non-held, consistent with the SDK live-filter decision above.
- **Prep/ready nudge (gates on `status==='preparing'`, `index.js`)**: **LEAVE — already correct.** A `completed` order gets no prep nudge (it's not preparing).
...
`xpizza-kitchen/index.html` (K1,K2) · `xpizza-dashboard/index.html` (D1–D8) · `xpizza-dispatch/index.html` (DI1–DI4) · `xpizza-track/index.html` (T1–T3) · `xpizza-orders/account.js` (A1) · `la-musa-orders/account.js` (A2) · `xpizza-functions/index.js` (F1, F2 — the ONLY functions file) · backfill script. **NOT touched (all deliberate LEAVE, §3B):** RTDB rules; `xpizza-delivery.js` / `order-filter.js` (the 5-copy SDK live-filter — `completed` stays live like `delivered`); `xpizza-functions/ready-time-quality.js` (shadow-monitor TERMINAL set); `xpizza-reference/test-harness.html` (non-prod); the driver app; all §3C machinery.
