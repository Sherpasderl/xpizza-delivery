...
- **Bounded (codex R1 #4):** per-field `.validate` as above (lat/lng range-checked, label/detected non-empty & length-capped), `$other:false` on the address child. **Max-count (R6):** RTDB rules have no child-count function (`numChildren()` is client-SDK only, not on the rules `RuleDataSnapshot` — emulator-confirmed it won't compile/deploy), so the **≤10-address cap is enforced CLIENT-side** (`saveAddress`, Task B3), not in rules. `addresses` is owner-only + per-field validated, so an owner over-saving their OWN addresses is a benign nuisance, not an exposure.
...
- `user_profiles/{uid}/addresses` — replace `".validate": false` with an owner-only structured store, using **`.validate` only** (the parent `$uid` `.write` already gates owner + tombstone + server-truth `hasChildren`; do NOT add a child `.write`):
  - `addresses` (parent): **no `.validate`** (R6) — the ≤10 cap moved CLIENT-side (RTDB rules can't count children); the `$addrId` child validates below fully govern each address.
