...
**Status:** DESIGN (rev 3, folds grill R1 [9] + R2 [5]) — awaiting re-grill, then owner review → plan → build → money-gate → owner-run gated cutover.
**Date:** 2026-09-09
...
- **Exposure representation + deterministic resolver (grill R2 #3).** Define ONE authoritative representation and a single deterministic resolution order — not a mix of per-extra/per-category/per-item maps with undefined precedence. Specify: **explicit deny precedence** (an exclusion like Nutella's must override a category-provided extra — an empty item map cannot exclude under La Musa's additive resolver at `la-musa-orders/index.html:1950`), **category-first ordering**, `rice_03` additions, and launcher/variant exposure behavior. Two implementations passing reference-validation must not be able to expose different option sets. **La Musa duplicate rejection is per submitted dish LINE** (`menu-pricing.js:193–203`) — the same extra on two different lines is valid; do not encode it as a global uniqueness rule.
