...
**⚠️ Surface 2 MECHANISM CORRECTION (on-device, vc25→vc26, advisor-approved 2026-07-13):** a `postDelayed`/main-looper heartbeat timer **does not fire in Doze** — the FGS keeps the *process* alive but not the *CPU*; `uptimeMillis` pauses during CPU-suspend (adb proof: process alive, standby EXEMPTED, no wakelock, `last_ping` frozen ~12 min). Doze-whitelist only honors a *held* wakelock. **E-v1 fix: acquire a shift-scoped partial `WAKE_LOCK`** (`"sherpa:shift-heartbeat"`) so the CPU stays awake and the 10s verbatim heartbeat fires. **Leak-safety is the hard gate:** acquire on tracking-start, **release on every stop path + `onDestroy`**, re-acquire on `START_STICKY` restart, idempotent (`setReferenceCounted(false)`/held-guard) — a leaked lock after clock-out means the CPU never sleeps = phone-killing drain. `WAKE_LOCK` is a **normal** permission (no Play declaration). FGS notification untouched; the vc25 heartbeat code otherwise unchanged. **Acceptance adds:** adb-confirm the wakelock is **RELEASED after clock-out** (battery-safety-critical) + a **battery-cost reading** over the locked stretch (= D's baseline). Chose the continuous wakelock over `setExactAndAllowWhileIdle` for E-v1 (guaranteed-correct; the alarm form's Honor-Doze throttle risk needs its own on-device proof → that's D's job). **D is now the committed battery payback, not optional-YAGNI.**
